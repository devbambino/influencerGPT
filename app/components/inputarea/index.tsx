...
export default function InputArea({ type, article, imagePreviewUrl, isLoading, onArticleChange, onFileChange }: InputAreaProps) {
    return (
            ...
            <h3 className="text-md leading-6 font-semibold">
                <span className="w-full text-xl text-green-500 font-bold">Step 2 / </span>
                {
                    type == "image" ?
                        ...
                        type == "link" ?
                            "Paste here the link of the page and we will generate for you an engaging text from it ready to be posted on social networks..." :
                            "Write or Paste here what you want to share and we will generate for you an engaging version of it ready to be posted on social networks..."
                }
            </h3>
            {
                type == "image" ?
                    <>
                        ...
                        {imagePreviewUrl && (
                            <div className="flex flex-row justify-center gap-2">
                                ...
                                <textarea className="w-full min-h-[100px] border rounded text-black" name="article" disabled={isLoading} id="text" placeholder="Enter the short description here..." value={article} onChange={onArticleChange} />
                            </div>
                        )}
                    </> :
                    type == "link" ?
                        <textarea className="w-full min-h-[50px] border rounded text-black" name="article" disabled={isLoading} id="text" placeholder="Enter the link here..." value={article} onChange={onArticleChange} /> :
                        <textarea className="w-full min-h-[200px] border rounded text-black" name="article" disabled={isLoading} id="text" placeholder="Enter the text here..." value={article} onChange={onArticleChange} />
            }
        </div>
    );
}